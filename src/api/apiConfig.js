...
API.interceptors.request.use(
   (config) => {
      ...
      if (recentCalls[endpoint] && now - recentCalls[endpoint].time < THROTTLE_WINDOW) {
         recentCalls[endpoint].count++;

         // If we're seeing rapid repeated calls, block them
         if (recentCalls[endpoint].count > 3) {
            console.warn(`Blocking repeated call to ${endpoint} (${recentCalls[endpoint].count} calls in 1s)`);
            ...
            throw new axios.Cancel(`Circuit breaker: Too many calls to ${endpoint}`);
         }
      } else {
      ...
      if (endpoint === "/settings/defaults") {
         console.warn("Intercepted request to non-existent /settings/defaults endpoint");
         const mockResponse = {
            data: {
               success: true,
               data: [],
...
API.interceptors.response.use(
   ...
   (error) => {
      ...
      if (error.response && error.response.status === 401) {
         console.error("Unauthorized access (401). Token may be invalid or expired.");
