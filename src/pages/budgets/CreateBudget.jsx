...
const CreateBudget = () => {
   ...
   const fetchExistingBudgets = async () => {
      try {
         ...
         const response = await fetch("http://localhost:5000/api/v1/budgets?limit=5", {
            method: "GET",
            headers: {
         ...
         if (responseData && responseData.data && Array.isArray(responseData.data)) {
            ...
            setBudgetError("Failed to load existing budgets. Unexpected data format.");
            setExistingBudgets([]);
            return;
         }
         ...
         setBudgetError("Failed to load existing budgets. Please try again later.");
         setExistingBudgets([]);
      } finally {
         setLoadingBudgets(false);
      }
   ...
   const handleSubmit = async (e) => {
      ...
      try {
         ...
         if (response && (response.success || response.data || response._id)) {
            ...
            const categoryName = categories.find((c) => c._id === formData.category)?.name || "Selected category";
            setSuccessMessage(`Budget for ${categoryName} created successfully${budgetId ? ` (ID: ${budgetId})` : ""}`);
            ...
            const errorMessage = response?.message || "Failed to create budget. Please try again.";
   ...
   const testDirectApiCall = async () => {
      try {
         ...
         if (response.ok) {
            ...
            setError(`Direct API test failed: ${data.message || data.error || response.statusText}`);
         }
      } catch (err) {
            ...
            <div className="lg:col-span-2">
               ...
               <motion.div
                  ...
                  <div className="bg-gradient-to-r from-[#3d348b] to-[#7678ed] p-6">
                     ...
                     <p className="text-white/80 mt-1">Fill in the information below to create a new budget</p>
                  </div>
                  ...
                  {!isAuthenticated ? (
                     <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 m-4">
                        <div className="flex">
                           ...
                           <p className="text-yellow-700">
                              You need to be logged in to create budgets. Please log in first.
                           </p>
                        </div>
                     </div>
                  ) : !isAdmin ? (
                     <div className="bg-yellow-50 border-l-4 border-yellow-500 p-4 m-4">
                        <div className="flex">
                           <FaExclamationTriangle className="text-yellow-500 mt-1 mr-3 flex-shrink-0" />
                           <p className="text-yellow-700">Only administrators can create and manage budgets.</p>
                        </div>
                     </div>
                  ) : (
                     <div className="p-6">
                        <form onSubmit={handleSubmit}>
                           <motion.div
                              ...
                              <motion.div variants={itemVariants}>
                                 ...
                                 <p className="text-xs text-gray-500 mt-1">
                                    When budget usage reaches this percentage, a warning will be shown
                                 </p>
                              </motion.div>
                              ...
                              <motion.div variants={itemVariants}>
                                 ...
                                 <p className="text-xs text-gray-500 mt-1">
                                    When budget usage reaches this percentage, a critical alert will be shown
                                 </p>
                              </motion.div>
                           ...
                           {/* Button Group */}
                           <motion.div variants={itemVariants} className="mt-6 flex justify-end space-x-4">
                              <Link
                                 to="/admin/budgets"
                  ...
                  <div className="p-4">
                     ...
                     {loadingBudgets && existingBudgets.length === 0 ? (
                        ...
                        <div className="py-6 text-center">
                           ...
                           <p className="text-gray-500">No budgets found. Create your first budget using the form.</p>
                        </div>
                     ) : (
                  ...
                  <div className="p-4">
                     <ul className="space-y-3 text-sm">
                        <li className="flex">
                           ...
                           <p>
                              <span className="font-medium">Period:</span> Select month and year for your budget. Choose
                              "Annual Budget" for a yearly allocation.
                           </p>
                        </li>
                        <li className="flex">
                           <FaMoneyBillWave className="text-[#f35b04] mt-1 mr-3 flex-shrink-0" />
                           <p>
                              <span className="font-medium">Amount:</span> Total budget allocation in Swiss Francs (CHF)
                              for the selected period and category.
                           </p>
                        </li>
                        <li className="flex">
                           <FaChartLine className="text-[#f7b801] mt-1 mr-3 flex-shrink-0" />
                           <p>
                              <span className="font-medium">Thresholds:</span> Set warning (yellow) and critical (red)
                              levels for budget usage alerts.
                           </p>
                        </li>
                     </ul>
                     ...
                     <div className="mt-4 text-xs text-gray-500">
                        ...
                        <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                           <div className="flex h-full">
                              ...
                              <div className="h-full bg-[#f7b801]" style={{ width: "20%" }}></div>
                              <div className="h-full bg-[#f35b04]" style={{ width: "20%" }}></div>
                           </div>
                        </div>
